const dataSet = [
...

  {
    "id" : "2",
    "name" : "Wilburn Weber",
    "email" : "Bennett_Kreiger@yahoo.com"
  },
...
function numberToText(number) {
  ...
  if (number >= 1 && number <= numberWords.length) {
...
pages.forEach(page => {
  const pageNumber = parseInt(page.getAttribute('data-number'), 10);
  if (!isNaN(pageNumber)) {
    const textRepresentation = numberToText(pageNumber);
    page.setAttribute('data-text', textRepresentation);
  }
});
